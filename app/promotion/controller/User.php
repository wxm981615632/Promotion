...
use app\promotion\service\UserService;
use app\promotion\model\User as UserModel;
use think\facade\Db;
...
class User extends Admin {
	...
	function update(){
		if (!$this->request->isPost()){
			...
			$this->view->assign('info',checkData(UserModel::find($id)));
			return view('update');
		}else{
			$postField = 'id,realname,head';
			$data = $this->request->only(explode(',',$postField),'post',null);
	...
	function dumpData(){
		...
		try {
			...
			$list = UserModel::where(formatWhere($where))->order('id desc')->select();
			if(empty($list)) throw new Exception('没有数据');
			UserService::dumpData(htmlOutList($list),filterEmptyArray(array_unique($fieldInfo)));
		} catch (\Exception $e) {
						...
						//根据中文名称来读取字段名称
							if($data[$i + ($p-1)*$export_per_num]){
								foreach($data[1] as $key=>$val){
									$fieldInfo = db('field')->where(['name'=>$val,'menu_id'=>726])->find();
									if($val && $fieldInfo){
